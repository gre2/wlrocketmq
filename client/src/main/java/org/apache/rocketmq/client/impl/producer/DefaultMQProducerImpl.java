...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    public void start(final boolean startFactory) throws MQClientException {
        switch (this.serviceState) {
            case CREATE_JUST:
                ...
                // 注册Producer，向MQClientInstance注册，将当前生产者加入到MQClientInstance管理中，方便后续调用网络请求、进行心跳检测等。
    ...
    private SendResult sendKernelImpl(final Message msg, //
        ...
        if (brokerAddr != null) {
            //todo 使用vip好处? 是否使用broker vip通道。broker会开启两个端口对外服务。
            ...
            try {
                ...
                if (this.tryToCompressMessage(msg)) {
                    //设置消息的系统标记为MessageSysFlag.COMPRESSED_FLAG
                ...
                // hook：如果注册了消息发送钩子函数，则执行消息发送之前的增强逻辑，通过DefaultMQProducerImpl#registerSendMessageHook注册钩子处理类
                ...
                requestHeader.setUnitMode(this.isUnitMode());//是否是批量消息等
                ...
                switch (communicationMode) {
                    ...
                    case ONEWAY:
                    case SYNC:
                        //todo sendKernelImpl方法调用了MQClientAPIImpl类的sendMessage()
                        sendResult = this.mQClientFactory.getMQClientAPIImpl().sendMessage(
                            brokerAddr,
                            mq.getBrokerName(),
                            msg,
