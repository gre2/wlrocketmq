...
public class MQClientInstance {
    ...
    public static TopicPublishInfo topicRouteData2TopicPublishInfo(final String topic, final TopicRouteData route) {
        ...
        if (route.getOrderTopicConf() != null && route.getOrderTopicConf().length() > 0) { // 顺序消息
            ...
            List<QueueData> qds = route.getQueueDatas(); // 非顺序消息
            ...
            for (QueueData qd : qds) {
                ...
                if (PermName.isWriteable(qd.getPerm())) {
                    ...
                    if (!brokerData.getBrokerAddrs().containsKey(MixAll.MASTER_ID)) { // 找不到或没有找到Master节点，不处理队列信息
                        continue;
                    }
        ...
        synchronized (this) {
            switch (this.serviceState) {
                case CREATE_JUST:
                    ...
                    this.mQClientAPIImpl.start();//NettyRemotingClient实现类，
    ...
    private void startScheduledTask() {
        ...
        this.scheduledExecutorService.scheduleAtFixedRate(new Runnable() {
            ...
            public void run() {
                try {
                    MQClientInstance.this.updateTopicRouteInfoFromNameServer();//保证producer的failover
                } catch (Exception e) {
        ...
        this.scheduledExecutorService.scheduleAtFixedRate(new Runnable() {
            ...
            public void run() { // TODO 待读：ALL
                try {
                    MQClientInstance.this.persistAllConsumerOffset();//保留
                } catch (Exception e) {
        ...
        this.scheduledExecutorService.scheduleAtFixedRate(new Runnable() {
            ...
            public void run() { // TODO 待读：ALL
                try {
                    MQClientInstance.this.adjustThreadPool();//调整
                } catch (Exception e) {
            ...
            if (this.lockNamesrv.tryLock(LOCK_TIMEOUT_MILLIS, TimeUnit.MILLISECONDS)) {
                try {
                    ...
                    if (topicRouteData != null) {
                        ...
                        //路由信息发生变化更新MQClientInstance Broker地址缓存表
                        if (!changed) {
