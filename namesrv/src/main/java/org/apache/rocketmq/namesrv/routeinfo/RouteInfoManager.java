...
public class RouteInfoManager {
    ...
    /**
     * broker地址 与 filtersrv数组 Map(Broker上的FilterServer列表，用于类模式消息过滤)
     */
        ...
        try {
            try {
                ...
                if (null == brokerData) {
                    ...
                    HashMap<Long, String> brokerAddrs = new HashMap<Long, String>();//brokerId,brokerAddr
                ...
                /*
                * 如果Broker为Master，并且Broker Topic配置信息发生变化或者是初次注册，则需要创建或更新Topic路由元数据，填充topicQueueTable，
                ...
                * 如果该主题未创建并且BrokerConfig的autoCreateTopicEnable为true时，将返回MixAll.DEFAULT_TOPIC的路由信息。*/
                ...
                if (null != topicConfigWrapper && MixAll.MASTER_ID == brokerId) {
                    if (this.isBrokerTopicConfigChanged(brokerAddr, topicConfigWrapper.getDataVersion())//
                        ...
                        ConcurrentHashMap<String, TopicConfig> tcTable =
                            topicConfigWrapper.getTopicConfigTable();//<String, TopicConfig>
                        if (tcTable != null) {
                ...
                // 更新broker连接信息，更新BrokerLiveInfo，存活Broker信息表，BrokeLiveInfo是执行路由删除的重要依据。
    ...
    /*我们应该不会忘记scanNotActiveBroker在NameServer中每10s执行一次。
    *逻辑也很简单，遍历brokerLiveInfo路由表（HashMap），
    *检测BrokerLiveInfo的lastUpdateTimestamp上次收到心跳包的时间如果超过当前时间120s，
    *NameServer则认为该Broker已不可用，故需要将它移除，关闭Channel，
