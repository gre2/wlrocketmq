...
public class MappedFileQueue {
    ...
    /**
     * 最后flush到的位置offset，当前刷盘指针， 表示该指针之前的所有数据全部持久化到磁盘
     */
    private long flushedWhere = 0;
    /**
     * 最后commit到的位置offset，当前数据提交指针，内存中 ByteBuffer 当前的写指针，
     * 该值大于等于 flushedWhere。
     */
    ...
    /**
     * 根据消息偏移量 offset查找 MappedFile。 根据 offet查找 MappedFile直接使用 offset%­ mappedFileSize 是否可行?答案是否定的，
     * 由于使用了内存映射，只要存在于存储目录下的文件，都需要对应创建内存映射文件，如果不定时将已消费的消息从存储文件中删除，会 造成极大的内存压力与资源浪费，
     * 所有 RocketMQ 采取定时删除存储文件的策略，也就 是说在存储文件中， 第一个文件不一定是 00000000000000000000，因为该文件在某一时 刻 会被 删除，
